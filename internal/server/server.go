...
func New(cfg *config.Config, db *sqlx.DB, printCh chan<- PrintTask, static fs.FS) *Server {
	...
	r.Group(func(r chi.Router) {
		...
		r.Put("/api/products/{sku}", s.handleUpdateProduct)
		r.Delete("/api/products/{sku}", s.handleDeleteProduct)
		r.Get("/api/categories", s.handleListCategories)
		r.Get("/api/brands", s.handleListBrands)
